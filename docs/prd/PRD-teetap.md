...
- **Distribution.** `install.sh` fetched via curl installs the script to `~/.local/bin/teetap` (no sudo), pinned to the latest GitHub Release by default and overridable with a version environment variable. The install script warns if `~/.local/bin` is not on PATH.
- **Deliberate omissions.** No reader subcommands (`tail`/`grep` wrappers), no daemon, no log rotation beyond `--rotate`, no npm/pip packaging, no agent-identity coupling, no re-teeing of process-manager output.
